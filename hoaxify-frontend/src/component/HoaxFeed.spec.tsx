...
// };
//
const setup = (props?: any, state = loggedInStateUser1) => {
...
describe('HoaxFeed', () => {
    ...
    describe('Lifecycle', () => {
        ...
        it('calls loadHoaxes when it is rendered', () => {
            jest.spyOn(apiCalls, 'loadHoaxes').mockResolvedValue(mockEmptyResponse)
            setup();
        ...
        it('calls loadHoaxes without user parameter when it is rendered without user property', () => {
            const spy = jest.spyOn(apiCalls, 'loadHoaxes');
            spy.mockResolvedValue(mockEmptyResponse);
            setup();
            const parameter = spy.mock.calls[0][0];
    ...
    describe('Layout', () => {
        ...
        it('does not display no hoax message when the response has page of hoax', async () => {
            jest.spyOn(apiCalls, 'loadHoaxes').mockResolvedValue(mockSuccessGetHoaxesSinglePage)
            const {queryByText} = setup();
            await waitFor(() => {
                const message = queryByText('There are no hoaxes');
        ...
        it('displays spinner when loading the hoaxes', async () => {
            ...
            jest.spyOn(apiCalls, 'loadHoaxes').mockImplementation(mockImplementation)
        ...
        it('displays hoax content', async () => {
            ...
            const { queryByText } = setup();
            await waitFor(() => {
                const hoaxContent = queryByText('This is the latest hoax');
                expect(hoaxContent).toBeInTheDocument();
            })
        });

            it('displays Load More when there are next pages', async () => {
                jest.spyOn(apiCalls, 'loadHoaxes').mockResolvedValue(mockSuccessGetHoaxesFirstOfMultiPage)
